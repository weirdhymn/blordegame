...
- **The four approaches** (damage "types", §14.7) map onto stats + job-skills, so a horse's specialization *is* its battle role: **Confront** (STR/smithing), **Outwit** (INT/reading), **Soothe** (**kindness** — the Agreeableness trait ÷ `KINDNESS_STAT_DIV` — + performance), **Endure** (CON/athletics). DEX = turn order + Flee; WIS = healing + reading the foe. Soothe keying off kindness gives **Benevolent** horses a real combat role. Every existing job structure (Forge/Library/Stage/Track/Forager's Hut) trains a role.
...
- **Turn loop:** order by **DEX desc, ties by the seeded RNG**, recomputed each round; KO'd skip. Each roll derives from `(seed, round, turnIndex)` so a battle replays identically. The menu (per party turn) is **Attack · Item · Defend · Flee** (the **Skill** menu lands next). Resolution reuses `skillCheck`: a clean hit scales with the attacker's stat mod + crit; a miss still chips (`DMG_GLANCE`, cozy); Defend halves the next incoming hit; Flee is a DEX check (a miss only costs the turn).
- **Approaches & weaknesses (the tactical heart).** **Attack** expands to **Confront / Outwit / Soothe / Endure** — each a `skillCheck` on its stat (Soothe on **kindness**). An enemy is **weak** to one approach (×`COMBAT_WEAKNESS_MULT`) and may **resist** one (×`COMBAT_RESIST_MULT`); a resisted hit still lands for ≥1 (cozy — a wrong approach is weaker, never a punish). The foe's **tell** is surfaced (a readable hint, so it's never blind guesswork) alongside each party horse's per-approach value, so matching the right horse + approach to the tell is the core decision; the log narrates it (*"a weak point! 💥"* / *"it barely notices"*). The `EnemyDef` carries `weakness`/`resist`/`tell`; foes don't pick approaches (they just swing with `power`).
...
- **Entry:** v1 ships a **standalone "Sparring Ring"** (`POST /api/battle/start`, `/battle/:id/act`, `GET /battle/:id`; client `/spar`) so the loop is playable now. The real entry — a battle as a new adventure **Outcome type** (`battle?: enemyId`) with a run→battle handoff, a **region boss** (the *Hollow-Keeper*) debuting *with* its weakness puzzle — lands with the next layer.
- **Deferred (layered after playtest):** the **status** layer (Rattled/Heartened — `CombatStatus` is forward-compat, empty so far), **harmony** (bonded shared-defense / "Covered"), the **Skill** menu (Heal/Guard/Rally/Study), the run→battle handoff + **region boss** (the *Hollow-Keeper*, debuting with its own weakness puzzle), and combat-trains-horses (per-attack skill XP via the existing `grantSkillXp` path). Out of scope entirely: movement/positioning grid, MP/spell economy, classes-as-a-separate-system, large enemy/status libraries.
