...
- **The four approaches** (damage "types") map 1:1 onto stats + job-skills, so a horse's specialization *is* its battle role: **Confront** (STR/smithing), **Outwit** (INT/reading), **Soothe** (CHA/performance), **Endure** (CON/athletics). DEX = turn order + Flee; WIS = healing + reading the foe. Every existing job structure (Forge/Library/Stage/Track/Forager's Hut) trains a role.
...
- **Turn loop:** order by **DEX desc, ties by the seeded RNG**, recomputed each round; KO'd skip. Each roll derives from `(seed, round, turnIndex)` so a battle replays identically. The menu (per party turn) is **Attack · Item · Defend · Flee** (+ **Skill** and the four-approach *choice* in the next layer). Resolution reuses `skillCheck`: a clean hit scales with the attacker's stat mod + crit; a miss still chips (`DMG_GLANCE`, cozy); Defend halves the next incoming hit; Flee is a DEX check (a miss only costs the turn).
...
- **Deferred (post-minimum, layered after playtest):** the four-approach choice + **weaknesses** (the `weakness`/`resist`/`tell` fields are authored but unread by the minimum), the **status** layer (Rattled/Heartened — `CombatStatus` is forward-compat, empty in v1), **harmony** (bonded shared-defense / "Covered"), the **Skill** menu (Heal/Guard/Rally/Study), the run→battle handoff + boss, and combat-trains-horses (per-attack skill XP via the existing `grantSkillXp` path). Out of scope entirely: movement/positioning grid, MP/spell economy, classes-as-a-separate-system, large enemy/status libraries.
