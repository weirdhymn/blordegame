...
The deferred status layer (§9.4a/b) and the rest of each class's ability kit, built together — so combat has turn-to-turn **decisions** and the four classes play **differently**, not just "strong vs. different enemies." The inert `statuses` array finally carries state; each class gets a real second move; numbers live in `@blorse/balance` §9.4e. **Bounded pass:** kits + statuses for the existing four classes only — no new classes, gear, combat-XP, or enemy expansion.

- **The decision:** every class now has its signature **Attack** *plus* a class ability, so each turn is "hit now / set up / protect / support," not one button. The ability is **class-gated** (the engine refuses an off-class move, like Mend) and **scales off an existing stat** — the emergent-spec rule holds (a better-statted horse is the better class).
- **The four kits:** **Knight — Bulwark** (`guarding`: until its next turn, foes' single-target blows redirect onto it, soaked at Defend strength; a *sweep* still splashes — it can't cover everyone; CON makes it the best soak). **Wizard — Mark** (`exposed`: the party's hits against that foe land for ×`EXPOSED_MULT`; a sharper Wizard — high INT — holds it open a turn longer). **Rogue — Feint** (a light DEX chip that leaves the foe `rattled`). **Cleric — Rally** (`heartened`: the ally's own attacks land for ×`HEARTENED_MULT`; duration scales with kindness, mirroring Mend) — kept alongside Mend, so the Cleric has three moves.
- **The status set (four — each owned by one class, ticked down at the start of the affected combatant's turn):** `exposed` (foe takes more — Wizard's setup), `heartened` (ally deals more — Cleric's buff), `rattled` (foe's attacks ×`RATTLED_MULT`, it swings wide — Rogue's control), `guarding` (Knight redirect/soak). Two are pure **setup→capitalize** (a turn invests, a later turn cashes in — e.g. *Mark → Cleave* spikes; the headline combo stacks ×exposed ×weakness ×heartened, big but bounded — never a one-shot); two are protective control. `rattled` and `heartened` were the two kinds the schema already anticipated.
- **Cozy throughout:** foe statuses only ever *reduce* the threat or *amplify your* payoff; a rattled foe still lands ≥1 (no 0-damage whiff); no enemy ever debuffs the party. Combat stays a puzzle, not a threat.
- **Deterministic + tested:** statuses live in the persisted snapshot and tick on `(seed, round, turnIndex)` like everything else (same seed + same ability script → identical battle). Tests cover each ability's apply + stat-scaling, each status's capitalize + cozy bound, class-gating, and determinism.
- **Still deferred (unchanged):** harmony (bonded shared-defense), the run→battle status-carryover, combat-trains-horses. Out of scope as before: positioning grid, MP economy, class XP/leveling, multiclassing, gear, large enemy/status libraries.
...
### 7p The Post Office (the social layer gets a face)

The boarded-up Town façade opens (§7k's reservation, spent): **📮 The Post Office** (`/town/post`), berry-and-gold awning. Mail grows up: `GET /messages` letters now carry the **sender's herd name** (LEFT join — system letters survive a departed sender), a `read` flag, and a real timestamp; **walking into the page reads everything** (`POST /messages/read-all` — one stamp, no per-letter ceremony, StrictMode-safe); `/me` carries `unreadMail`, worn as a little berry number on the 🏘 Town nav tab and as the façade's live line. Compose + one-click **Reply** (prefills the recipient) live here; the Herd page's old raw-UUID message form retires to a pointer. **System letters** arrive from the Post Office itself (`fromHerd null` — migration 0022): the first is the **§10 promise finally delivered** — when another herd recruits a Tavern stray, the herd that FIRST encountered it gets a letter ("the stranger you met on the road found a home… it looked back once, in a good way"), quietly skipped when the finder recruits its own find (`sendSystemLetter` in messaging.ts; wired in `recruitFromTavern`). Address discovery still leans on shared herd ids — the Calling-Cards friends list remains the menu's next social step.

### 7q Calling Cards (the rolodex + sprite visits)

The UUID-pasting era ends. **`GET /contacts`** returns the address book, **derived at read** (nothing stored — `services/contacts.ts`): every herd you've exchanged **mail** with (system letters introduce nobody), **traded** with, or are tied to by **the road** — the same finder↔adopter tie the Post Office letter celebrates (`firstEncounteredBy`, both directions). One card per herd, freshest interaction first and labeling the `via`; departed herds drop off; capped at 50. Surfaced twice: the **Post Office compose** becomes a calling-card picker (manual id stays as fallback — writing someone once adds them), and the **Herd page's visit flow** grows one-click contact buttons. **Visits show real sprites**: profile highlights now carry `genotype`/`seed`/`glitch` for ADULTS ONLY — foals ship neither field (the §4.2 reveal-leak class), and the client paints them with a stock white silhouette. Six §7q checks pin the derivation (mail both ways, trade, road both ways, dedup-freshest, system-letter exclusion) and the no-leak rule.

### 7r The Mod Desk (moderation grows a face)

The last pre-beta blocker: moderation had a complete server surface and no UI. Now: **`/mod`** (🛡 nav link for `mod`/`admin` roles; the server 403s everyone else regardless) — stats tiles (users/herds/horses/open reports), the **report queue** with one-click **✓ Resolved / ✕ Dismiss** (`POST /api/mod/reports/:id/resolve` — reports used to stay `open` forever; the close is audited with the closing mod's id), and the admin-only **freeze/unfreeze** form (confirm-gated; "no harm lingers"). Player side, `POST /report` finally has callers: **⚑ report** affordances where players actually meet each other — on Post Office letters (reports the sender) and on visited herd profiles — with a `window.prompt` reason (one gesture, rate-limited 5/min server-side as before). Four new §7r checks: players can't close (403), bogus status 400, mod close 200, and closed reports leave the open queue + stats tile.

### 7s A Parcel, With String (gifting)

Letters can carry items. A parcel is warmth-sized by design — `PARCEL_MAX_STACKS = 5` kinds, `PARCEL_MAX_QTY = 20` apiece (balance) — and **moves atomically with the send**: one transaction consumes from the giver (the conditional-decrement kernel), grants to the recipient, and writes the letter; a short stash *snaps the string* (409 `parcel_short`) and nothing moves, nothing sends. Any real item may travel (pure transfer, nothing minted — rares included; the existing 20/min mail rate limit is the throughput cap); gifts are audited (`gift`). The letter records the parcel as its gift tag (`messages.parcel` jsonb, migration 0023) — the items are already in the recipient's stores when they read it ("📦 Came with: Carrot ×3 — already in your stores."). Compose grows a small packer (pick from your stores, up to five removable chips); system letters never carry parcels. Validation is layered: transport bounds in the route schema (`stackArray`), item-reality + dedup in the service, ownership at the atomic consume. Five §7s checks: the atomic move, the gift tag, snap-nothing-moves, fictional-item/freight 400s, parcel-less letters unchanged. Feeds the garden/cooking economy across herds — no new currency, pure warmth.

### 7t Lines in Good Ink (the pedigree gets portraits)

The no-shared-ancestor rule (§5.4a) — the game's most original mechanic — becomes something you can SEE. `GET /horses/:id/pedigree` nodes now carry `genotype`/`seed`/`glitch` **for adults only**, and the depth-3 text list on the horse sheet becomes a **portrait tree**: scale-1 sprites in linked cards, each generation indented behind a dashed spine, captioned "Two horses may breed only if their trees share no name." Two leak fixes ride along: the old pedigree printed a **foal's real coat name** into the tree (`resolve().displayName` unmasked — the §4.2 class, now `'Foal'` + no render fields; the client paints the stock white silhouette), and stamps are viewer-scoped. **✒ in good ink:** when a session rides the request, ancestors whose reveal fulfilled a goal in the VIEWER's studbook wear the stamp (`stampedHorseIds` in studbook.ts; anonymous requests get a stamp-free tree). Four §7t checks: foal-subject masking, adult portraits, exactly-the-stamped-ancestor, anonymous-tree-unstamped.

### 7u Something New in the Woods (the first gene drop — Mushroom)

The live-service muscle (§5.6) fired for the first time, per GENE_DROP_RUNBOOK.md — shipped **dark** (commit 1), then the dial turned up (commit 2). **Mushroom** (`My`, recessive `my`): a real, rare equine dilution — sepia-taupe over red pigment, **expressed only on a plain chestnut base** (hidden under gray/white/cream; carried silently on black-based coats). **Architecture, the part the runbook learned:** the locus is a pure DATA entry in `vendor/data.js` (LOCI + displayOrder + rollFrequencies `my: 0` = the global dark gate + `introduced`/`rarity` tags) — but its LOOK and NAMING live in the **facade**, not the engine: `resolve`/`breedFoal` post-pass for the name ("Mushroom", or prefixed), the §4.3a procedural **sepia swatch** (flows to the renderer through `varySwatch` — zero render-core changes), modifier + note; `enumerateColors` appends the reserved Field Guide slot facade-side (catalog 71 → **72**). **One flagged engine-side line:** `OFF` gained the row `My: 'MyMy'` — the static baseline table the runbook's own backward-compat promise depends on; without it, legacy genotypes crash in `breedFoal`/`formatGenotype`. No engine logic touched; two frozen content pins in the vendored tests updated (genotype-space ×3; the canonical OFF string) with all 338 differential/ratio guards passing unchanged. **The dial:** Weird Woods only — `My: { my: 0.05 }` (wild expressed mushrooms are genuinely rare; breeding carriers is the deliberate path). Integration: the Studbook gains the Master goal **Something New** (14 goals; ~4,650 ⬡ lifetime), `plain()` excludes mushroom so A Copper Penny isn't fooled by sepia, and the Field Guide's top milestone moved to 72. Punnett odds don't yet enumerate the locus (the factored key list is engine-internal) — noted as the drop's v1 limit; carriers breed true regardless.

### 7v The Tundra (the fourth region)

The promised cold place (§13/§14.7), pure content on the per-region pattern — zero engine work. **Region:** tier 4, recommended power 10, unlocked by the new chain quest **Through the Woods** (`into-the-dunes` → forage WW once + complete two WW expeditions, 300 ⬡); loot leans ore (6) / timber (2) / bone (2) / rare-gem (1). **Genetics weather:** leans *relatively* gray (`G: 0.12` vs the 0.05 global base — a lean, not a flood, per §14.7), silver `Z: 0.10`, roan `Rn: 0.08`, cream/pearl light; Mushroom stays Woods-only. **Five omens** (Diamond Dust int · The Long Blue Hour wis · Snow Silence cha · Firm Crust dex · Glasswind +ore). **Enemies:** the Frostbitten Stump (weak confront / resists outwit) and the Sleet-Wisp (weak soothe / **resists skirmish**) for mid-run scraps — and the Keeper, **the Glacier-Shepherd**: weak to a Rogue's **Skirmish** (quick feet thread ten-thousand-year seams), resists Soothe ("it has been calm for an age; your kindness is redundant") — **the Rogue's boss at last**, completing the class↔Keeper square (GG soothe · DD confront · WW outwit · TN skirmish). Boss reward matches keeper scale incl. saffron + fairy dust. **Expeditions** (two pool + Keeper, structurally distinct): *The Aurora Road* — a long lit night-walk, no combat, a befriendable stranger under the pooled lights at its far end; *The Hot Spring Hollow* — short, warm, social, with an optional Sleet-Wisp scrap as its deep push; *The Glacier-Shepherd* — the earned two-scene approach with the §9.4c battle handoff and the dignified bow-out. Scripts append LAST in the barrel (existing regions' seeded draws stay byte-identical). The Keeper is prestige + a second fairy-dust well, deliberately NOT added to `HERD_TIERS` (the ladder's gates stand). Eleven §7v checks: integrity (region/loot/gray-lean/quest-chain/omens/pool/Keeper-weakness), locked-before-quest, the chain completing on its own events, and an Aurora Road walk that banks.
