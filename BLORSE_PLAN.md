...
- **PRODUCTION TODO.** Run state currently lives in an **in-memory per-process store**; swap for a small `adventureRuns` table so runs survive restarts / multiple instances. Fatigue is flavor-only (no hard cooldown yet).
